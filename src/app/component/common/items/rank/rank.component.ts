...
import { RankInfo } from '@app-models/survey-info';
...
export class RankComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    public info: RankInfo = new RankInfo();    
    ...
    public extra_width = 20 + 10 + 40 + 60;
    ...
    ngOnInit() {
        this.info = lodash.cloneDeep((this.item.content as CommonItemContent<RankInfo>).info.value);        
    }

    ngAfterViewInit() {        
        this.parent = (this._elementRef.nativeElement as HTMLElement).parentElement.parentElement;        
        this._ranks = lodash.range(1,this.info.texts.length+1);
        this._subs = [            
    ...
    onAdd(event: MouseEvent) {
        ...
        this.info.values.push(0);
        this._ranks.push(this._ranks.length+1);
        this.editable = true;        
        this._changeDetectRef.detectChanges();        
        this.setItemResize();
    }
    ...
    onRemove(i: number, event: MouseEvent) {
        ...
        this._ranks = lodash.range(1,this.info.values.length+1);
        this._changeDetectRef.detectChanges();
        this.setHeight();
    }

    emmitItemChange(extraHeight: number = 0) {        
        let content = this.item.content as CommonItemContent<RankInfo>;        
    ...
    selectRank(event: number, i: number = 0) {        
        this.info.values = this.info.values.map(value=>value==event ? 0 : value);
        this.info.values[i] = event;
        this._changeDetectRef.detectChanges();
    }
