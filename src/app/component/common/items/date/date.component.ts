...
import { DateInfo } from '@app-models/survey-info';
...
export class DateComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    public info: DateInfo = new DateInfo();
    ...
    ngOnInit() {
        this.info = lodash.cloneDeep((this.item.content as CommonItemContent<DateInfo>).info.value);
    }
    ...
    setHeight() {
        ...
        if (this.editable)
            this.parent.style.height = questionTitleHeight + optionContainerHeight + this.container_padding + 35 + 'px';
        else
    ...
    onRemove(event: MouseEvent, i: number) {
        ...
        this.info.dates.splice(i, 1);
        this._changeDetectRef.detectChanges();
        this.setHeight();
    }

    emmitItemChange(extraHeight: number = 0) {
        let content = this.item.content as CommonItemContent<DateInfo>;
        ...
        clone_width += this.container_padding + 20;
