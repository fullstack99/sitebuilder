...
export class EditAppointmentComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild('dateSwiper') monthSwiper: any;
	...
	showLocationValue = (l: LocationInfo) => l ? l.address1 + ', ' + l.address2 + ', ' + l.city + ', ' + l.province : '';
	...
	ngOnInit() {
		...
		this.provider = lodash.find(this.providers, {uid: this.form.controls.providerUid.value});
	...
	setTimes() {
		...
		if (this.provider && this.provider.hoursOfOperation) {
